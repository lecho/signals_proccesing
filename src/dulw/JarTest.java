...
public class JarTest {
	...
	public static void main(String[] args) throws FunctionEvaluationException {
		...
		for (double i = 0; i < 26; i += 0.05)
			dataSet2.add(i, i * Math.sin(i * 3) + 35);
		for (double i = 0; i < 26; i += 0.3)
			dataSet3.add(i, Math.tan(i) + 45);
		...
		//UnivariateRealFunction func = SignalsFactory.createSinusSignal(5, 2, 4);
		//XYSeries xs = SignalsUtils.packLinearFunctionToXYSeries(func, 0, 100);
		//XYSeriesCollection xsc = new XYSeriesCollection(xs);
		// List<double[]> table = SignalsUtils.simplingSignal(func, 0, 10, 10);
		//HistogramDataset dataset = SignalsUtils.toHistogram(xs, "Description",
			//	10);
		// tworzenie wykresu
		JFreeChart lineGraph = ChartFactory.createXYLineChart("Tytul wykresu", // Title
		...
		ChartFrame frame = new ChartFrame(
				"Szybkie wyswietlanie wykresu - klasa ChartFrame", lineGraph);
		frame.pack();
		frame.setVisible(true);

		// Zapisywanie wykresu do pliku JPG:
		// try {
		// ChartUtilities.saveChartAsJPEG(new File("LineGraph.jpg"), lineGraph,
		// 800, 600);
		// } catch (Exception e) {
		...
		JFrame jframe = new JFrame(
				"Przeskaluj okno oraz kliknij prawym klawiszem myszy na obu wykresach...");
